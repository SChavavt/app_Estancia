...
TAB2_SMARTSCORE_ALIASES: dict[str, tuple[str, ...]] = {
    # Instant Noodles
    "maruchan": ("maruchanramensaborpollo",),
    "neu": ("nongshimneogurispicyseafood", "neogurispicyseafood", "neoguri"),
    "neoguri": ("nongshimneogurispicyseafood", "neogurispicyseafood"),
    "shinramyun": ("nongshimshinramyun", "shinramyun"),
    # Mac & Cheese
    "annies": ("anniesshellswhitecheddar",),
    "maccheese": ("kraftmacaronicheesedinner", "kraftmacandcheese"),
    "kraft": ("kraftmacaronicheesedinner",),
    "velveeta": ("velveetaoriginalshellscheesemicrowavecups", "velveetaoriginalshellscheese"),
    # Ready to Eat
    "tuna": ("wildplanetwildtunapastasalad",),
    "indian": ("kitchensofindiavarietypack",),
    "chicken": ("starkistchickencreationschickensalad",),
    "jacklinks": ("jacklinksbeefjerkyoriginal",),
}
...
def _load_user_smartscore_map(user_name: str) -> tuple[dict[str, float], str]:
    ...
    if mapping:
        alias_entries: dict[str, float] = {}
        for alias_key, canonical_candidates in TAB2_SMARTSCORE_ALIASES.items():
            if alias_key in mapping:
                continue
            for candidate in canonical_candidates:
                if candidate in mapping:
                    alias_entries[alias_key] = mapping[candidate]
                    break
        if alias_entries:
            mapping.update(alias_entries)
